...
class TransactionRecordMixin:
    """Transaction record operations mixin (auto-generated)."""
    ...
    async def get_transaction_record_by_request_id(
        ...
        response = await self._transaction_records_api.get_transaction_record_by_request_id(
            wallet_uuid, request_id, ext
        )
        return self._extract_result(response)

    async def get_transaction_record_by_uuid(
        self: "BaseClient", record_uuid: str, ext: bool | None = None
    ) -> Any:
        """Get transaction record by record id"""
        response = await self._transaction_records_api.get_transaction_record_by_uuid(
            record_uuid, ext
        )
        return self._extract_result(response)
