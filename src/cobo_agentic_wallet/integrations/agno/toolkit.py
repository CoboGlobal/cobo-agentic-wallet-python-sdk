...
class CoboAgentWalletTools(Toolkit):
    ...
    def _invalid_arguments_payload(
        self, tool_name: str, exc: Exception, kwargs: dict[str, Any]
    ) -> str:
        """Mirror the OpenAI integration's structured ``INVALID_TOOL_ARGUMENTS`` envelope.

        LLMs self-correct far more reliably when they see a JSON object with
        ``required`` / ``received`` fields than when they see a free-form string.
        """

        definition = self._tool_definitions.get(tool_name)
        required = (
            definition.parameters.get("required", [])
            if definition and isinstance(definition.parameters, dict)
            else []
        )
    ...
    async def _invoke_async(self, tool_name: str, **kwargs: Any) -> Any:
        ...
        try:
            ...
            # Surface handler-level validation errors (missing / malformed kwargs,
            # empty spec, etc.) as a structured JSON payload so the LLM can retry
            # with the correct shape instead of falling back to asking the user.
            return self._invalid_arguments_payload(tool_name, exc, kwargs)
    ...
    def submit_pact(
        ...
        ``spec`` is declared as optional at the Python layer so Agno's pydantic
        ``@validate_call`` wrapper does not reject tool invocations that omit it;
        the canonical handler validates the shape and returns a structured
        ``ValueError`` with a copy-pasteable example when the payload is missing
        or empty, which the LLM can read and retry.
        """
    ...
    def transfer_tokens(
        ...
        ``request_id`` is intentionally not exposed to the agent: LLMs tend to
        re-use hard-coded strings (e.g. ``"req1"``, ``"req2"``) across calls,
        which collide on the server's idempotency check. The tool neither
        accepts nor auto-generates one — behavior matches ``caw tx transfer``
        when the ``--request-id`` flag is omitted. Callers that need
        idempotency should invoke ``WalletAPIClient.transfer_tokens`` directly.
        """
    ...
    async def asubmit_pact(
        ...
        See :meth:`submit_pact` — ``spec`` is optional at this layer to avoid
        Agno's pydantic pre-validation crashing on LLM omissions; the canonical
        handler returns structured guidance when it is missing.
        """
