...
def get_cobo_tools(
    ...
    *,
    ...
    Shortcut that discards the toolkit; prefer :func:`get_cobo_tools_with_toolkit`
    when the caller needs to clean up pact-scoped clients via ``toolkit.aclose()``.
    """

    _, tools = get_cobo_tools_with_toolkit(
        client, include_tools=include_tools, exclude_tools=exclude_tools
    )
    return tools


def get_cobo_tools_with_toolkit(
    client: WalletAPIClient,
    *,
    include_tools: list[str] | None = None,
    exclude_tools: list[str] | None = None,
) -> tuple[AgentWalletToolkit, list[FunctionTool]]:
    """Return the filtered ``AgentWalletToolkit`` alongside its FunctionTools.

    Exposing the toolkit lets callers run ``await toolkit.aclose()`` so that any
    pact-scoped ``WalletAPIClient`` instances created during a run are released
    cleanly, preventing aiohttp "Unclosed client session" warnings.
    """
    ...
    tools = build_function_tools(base_toolkit, denial_recorder=_record_policy_denial)
    return base_toolkit, tools
...
def create_cobo_agent(
    ...
    **agent_kwargs: Any,
    ...
    toolkit, tools = get_cobo_tools_with_toolkit(
        client,
        include_tools=include_tools,
        exclude_tools=exclude_tools,
    )
    ...
    agent = Agent(**resolved_agent_kwargs)
    # Attach the toolkit so callers can release pact-scoped clients after the run:
    #   toolkit = getattr(agent, "_caw_toolkit", None)
    #   if toolkit is not None: await toolkit.aclose()
    agent._caw_toolkit = toolkit  # type: ignore[attr-defined]
    return agent
